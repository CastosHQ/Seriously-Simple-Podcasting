	...
	public function episode_meta( $episode_id = 0, $context = 'content' ) {
		...
		if ( $file ) {
			...
			if ( ! ssp_check_if_podcast_has_shortcode( $episode_id, 'ss_player' ) ) {
				...
				if ( $show_player ) {
					$meta .= '<div class="podcast_player">' . $this->player->media_player( $file, $episode_id, $player_style ) . '</div>';
				}
				...
				foreach ( $content_items as $item ) {
					...
					switch( $item ) {
						...
						case 'player':
							...
							$html .= '<div class="podcast_player">' . $this->player->media_player( $file, $episode_id, "large" ) . '</div>' . "\n";
							break;
			...
			foreach ( $content_items as $item ) {
				...
				switch( $item ) {
					...
					case 'player':
						...
						$html .= '<div class="podcast_player">' . $this->player->media_player( $file, $episode_id, $style ) . '</div>' . "\n";
						break;
