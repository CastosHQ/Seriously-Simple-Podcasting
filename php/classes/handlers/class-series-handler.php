...
class Series_Handler implements Service {
	...
	public function __construct( $notices_handler ) {
		...
		add_action( 'ssp_triggered_podcast_sync', array( $this, 'update_sync_status_on_sync' ), 10, 3 );
	}
	 ...
	 */
	public function update_sync_status_on_sync( $podcast_id, $response, $sync_status ) {
		$prev_status = $this->get_sync_status( $podcast_id );

		$this->notices_handler->add_predefined_flash_notice( Admin_Notifications_Handler::NOTICE_IS_SYNCING );

		if ( $prev_status != $sync_status ) {
			if ( Sync_Status::SYNC_STATUS_SYNCING === $sync_status ) {
				$this->notices_handler->add_predefined_flash_notice( Admin_Notifications_Handler::NOTICE_IS_SYNCING );
			} else {
				$this->notices_handler->remove_predefined_flash_notice( Admin_Notifications_Handler::NOTICE_IS_SYNCING );
			}
		}

		return $this->update_sync_status( $podcast_id, $sync_status );
	}
